...
api=$name-api
bot=$name-bot
network_created=0 database_created=0 api_created=0 bot_created=0
cleanup() {
  local status=$?
  trap - EXIT
  if [ "$status" != 0 ]; then
...
start_api() {
  docker run -d --name "$api" --network "$network" --entrypoint /app/bin/core-api \
    ...
    -e CORE_API_TOKEN=synthetic-api-token "$image" >/dev/null
  api_created=1
  wait_api
}
probe_bot() {
...
start_bot() {
  docker run -d --name "$bot" --network "$network" \
    ...
    -e DISCORD_BOT_TOKEN=synthetic -e CLIENT_ID=123 -e NODE_ENV=production \
...
probe 503 /health
probe_bot 503
echo 'core-image: shared image, API/Bot readiness, authentication, schema mismatch, API/DB outage and dump/restore passed'
