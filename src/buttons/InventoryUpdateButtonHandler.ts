...
export class InventoryUpdateButtonHandler extends BaseButtonHandler {
  ...
  private buildReply(items: InventoryItem[], page: number): Record<string, unknown> {
    const totalPages = Math.ceil(items.length / InventoryUpdateButtonHandler.pageSize);
    const currentPage = Math.min(page, Math.max(totalPages - 1, 0));
    const pageItems = items.slice(
      currentPage * InventoryUpdateButtonHandler.pageSize,
      (currentPage + 1) * InventoryUpdateButtonHandler.pageSize
    );
    ...
    const components: unknown[] = [
      {
        ...
        components: [
          {
            type: 3,
            custom_id: `inventory_update_select_${currentPage}`,
      ...
      components.push({
        ...
        components: [
          ...(currentPage > 0
            ? [{
              type: 2,
              custom_id: `inventory_update?page=${currentPage - 1}`,
              label: '前へ',
              style: 2
            }]
            : []),
