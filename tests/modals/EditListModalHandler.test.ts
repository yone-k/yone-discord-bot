...
const setup = (customId = 'edit-list-modal:123:456:7', text = '牛乳,,,'): {
  ...
  const handler = new EditListModalHandler(new Logger(), repo as any, metadata as any);
  const context = { interaction: { customId, channelId: '123', user: { id: '456' }, fields: { getTextInputValue: () => text }, client: { channels: { fetch: vi.fn() } } } } as any;
...
describe('編集モーダルDB保存', () => {
  ...
  it('競合は再読込を促し再保存しない', async () => { const { repo, handler, context } = setup(); repo.save.mockRejectedValue(new RepositoryError('conflict', 409)); const result = await (handler as any).executeAction(context); expect(result.success).toBe(false); expect(result.message).toContain('開き直'); expect(context.interaction.client.channels.fetch).not.toHaveBeenCalled(); });
...
it('ボタンで開いたCSVを保存し、TSでは共有カードを再描画しない', async () => {
