...
function setup(input = '米,1.234,0.0123'): any {
  ...
  const handler = new Handler(new Logger(), undefined, undefined, repository as any);
  const interaction = { customId: 'remind-task-inventory-modal:456:7', channelId: '3', client: { channels: { fetch: vi.fn() } }, fields: { getTextInputValue: (): string => input } };
  return { task, repository, handler, interaction };
}
describe('RemindTaskInventoryModalHandler', () => {
  it('sends all raw quantities together without rendering the shared card', async () => {
    const x = setup();
  ...
  it('leaves inventory and referencing task rendering to the committed backend output', async () => {
    const x = setup('米,5,0\n新規,1');
