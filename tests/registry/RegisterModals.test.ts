...
describe('RegisterModals', () => {
  ...
  describe('registerAllModals', () => {
    ...
    it('RemindTaskUpdateModalHandlerが正しく登録される', () => {
      registerAllModals(modalManager, logger);
      const registeredHandlers = modalManager.getRegisteredHandlers();
      const handler = registeredHandlers.find(h => h.getCustomId() === 'remind-task-update-modal');
    ...
    it('RemindTaskCompleteModalHandlerが正しく登録される', () => {
      registerAllModals(modalManager, logger);
      const registeredHandlers = modalManager.getRegisteredHandlers();
      const handler = registeredHandlers.find(h => h.getCustomId() === 'remind-task-complete-modal');
      expect(handler).toBeDefined();
      expect(handler?.constructor.name).toBe('RemindTaskCompleteModalHandler');
    });

    it('RemindTaskDeleteModalHandlerが正しく登録される', () => {
      registerAllModals(modalManager, logger);
      const registeredHandlers = modalManager.getRegisteredHandlers();
      const handler = registeredHandlers.find(h => h.getCustomId() === 'remind-task-delete-modal');
      expect(handler).toBeDefined();
    ...
    it('RemindTaskInventoryModalHandlerが正しく登録される', () => {
      registerAllModals(modalManager, logger);
      const registeredHandlers = modalManager.getRegisteredHandlers();
      const handler = registeredHandlers.find(h => h.getCustomId() === 'remind-task-inventory-modal');
      expect(handler).toBeDefined();
      expect(handler?.constructor.name).toBe('RemindTaskInventoryModalHandler');
    });

    it('InventoryAddModalHandlerが正しく登録される', () => {
      registerAllModals(modalManager, logger);
      const registeredHandlers = modalManager.getRegisteredHandlers();
      const handler = registeredHandlers.find(h => h.getCustomId() === 'inventory_add_modal');
      expect(handler).toBeDefined();
      expect(handler?.constructor.name).toBe('InventoryAddModalHandler');
    });
    ...
    it('登録されるハンドラー数が期待する値と一致する', () => {
      ...
      // RemindTaskDeleteModalHandler, RemindTaskAddModalHandler, RemindTaskInventoryModalHandler,
      // InventoryAddModalHandler, InventoryUpdateModalHandler, InventoryDeleteModalHandler
