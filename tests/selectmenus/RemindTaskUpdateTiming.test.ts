...
describe('繰り返し更新の初回応答', () => {
  ...
  it('タスク取得を保留しても更新ボタンは即座にdeferする', async () => {
    ...
    const log = logger();
    const handler = new RemindTaskUpdateButtonHandler(log, undefined, undefined, repository as any, { buildUpdateSelectionComponents: vi.fn().mockResolvedValue([]) } as any);
    ...
    expect(vi.mocked(log.debug).mock.calls.map(call => (call[1] as any).stage)).toEqual(['received', 'initial_response', 'task_fetch', 'selection_build']);
  });
  ...
  it.each(['no-link', 'empty', 'missing-id'])('在庫参照 %s の既存表示と不要な取得の省略を維持する', async mode => {
    ...
    await new RemindTaskUpdateSelectMenuHandler(logger(), undefined, metadata as any, { findTaskByMessageId: vi.fn().mockResolvedValue(task) } as any, { updateTaskMessage: vi.fn().mockResolvedValue({ success: true }) } as any, inventory).handle({ interaction } as any);
  ...
  it.each(['basic', 'advanced', 'inventory'])('%s の再描画が3秒を超えてもフォームと別操作を待たせない', async selection => {
    vi.useFakeTimers();
    const log = logger();
    const manager = { updateTaskMessage: vi.fn(() => new Promise(resolve => setTimeout(() => resolve({ success: true }), 4000))) };
    const handler = new RemindTaskUpdateSelectMenuHandler(log, undefined, undefined, { findTaskByMessageId: vi.fn().mockResolvedValue(taskFixture()) } as any, manager as any);
    ...
    expect(stages.filter(x => x.messageId === 'msg-1').map(x => x.stage)).toEqual(['received', 'task_fetch', 'modal_prepare', 'initial_response', 'message_restore']);
    expect(stages.find(x => x.stage === 'message_restore')).toMatchObject({ stageElapsedMs: 4000, totalElapsedMs: 4000, replied: true, deferred: false, errorCode: null });
  });
  ...
  it.each(['false', 'core', 'unexpected'])('フォーム表示後の再描画失敗 %s はログのみ', async failure => {
    const log = logger();
    const manager = { updateTaskMessage: failure === 'false' ? vi.fn().mockResolvedValue({ success: false }) : vi.fn().mockRejectedValue(failure === 'core' ? new CoreApiError('unavailable', 503) : new Error('broken')) };
    const interaction = formInteraction('remind-task-update-select:msg-1');
    await new RemindTaskUpdateSelectMenuHandler(log, undefined, undefined, { findTaskByMessageId: vi.fn().mockResolvedValue(taskFixture()) } as any, manager as any).handle({ interaction } as any);
    expect(interaction.showModal).toHaveBeenCalledOnce();
    expect(interaction.reply).not.toHaveBeenCalled();
    expect(interaction.followUp).not.toHaveBeenCalled();
    expect(interaction.editReply).not.toHaveBeenCalled();
    expect(log[failure === 'unexpected' ? 'error' : 'warn']).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({ stage: 'message_restore', replied: true, errorCode: failure === 'core' ? 'unavailable' : null }));
  });
  it.each([false, true])('在庫20件を3通信で準備し、累積2秒の予算を守る（期限超過=%s）', async timeout => {
    ...
    const manager = { updateTaskMessage: vi.fn().mockResolvedValue({ success: true }) };
    const interaction = formInteraction('remind-task-update-select:msg-1', 'inventory');
    const log = logger();
    const pending = new RemindTaskUpdateSelectMenuHandler(log, undefined, metadata as any, repository as any, manager as any, inventoryRepository as any).handle({ interaction } as any);
    await vi.advanceTimersByTimeAsync(timeout ? 2000 : 1800);
    await pending;
    expect(fetcher).toHaveBeenCalledTimes(3);
    if (timeout) {
      expect(interaction.reply).toHaveBeenCalledOnce();
      expect(manager.updateTaskMessage).not.toHaveBeenCalled();
